...
- Голова — спрайт (`headArt`/`headPix`, сетка 26×28, кэш на бойца, выражение
  и затемнение). Собирается в рантайме из плоских заливок: жёсткая альфа,
  сведение к палитре из девяти цветов, обводка в один пиксель. Правила
  сборки, которые уже стоили циклов: силуэт (череп, челюсть, нос) — ТОЛЬКО
  раздельными заливками, один `Path2D` с подпутями противоположной намотки
  выедает дыру; детали держит `globalCompositeOperation = 'source-atop'`,
  а не `clip()`; нос обязан входить в силуэт, иначе он рисуется поверх глаза;
  линия волос выше брови, иначе лоб съеден и лицо читается шапкой. Смотреть
  спрайты листом при увеличении ×14 (`scratchpad/sheet.js`), не через сцену.
- Пиксельный конвейер: кадр рисуется в буфер (`pixBegin/pixEnd`, тексель
  `PIXEL` css-px, `PIX_LEVELS` ступеней яркости с сохранением оттенка), на
  время кадра подменяются `cx` и `DPR`; `drawScene` — вся сцена и врезки,
  `filmPass` ложится поверх резко. `PIXEL = 0` выключает.
- Снимок врезки: время сцены во врезке замедлено, а её таймер `cut.t` идёт
  в реальных мс; снимать по `cut.t`, заморозив цикл
  (`requestAnimationFrame = function(){}`) перед скриншотом. Готовые
  картинки врезок отключаются `CUT_ART = null; cutImgs = null` — так виден
  запасной процедурный вариант, который игрок видит до загрузки хвоста.
