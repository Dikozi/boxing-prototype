...
- Случайную величину нельзя мерить абсолютным порогом. Щелчки хруста —
  шум: их пик гуляет, и порог «выше четверти максимума» давал то два, то
  четыре (CI падал на правке, не касавшейся звука). Мерить структуру
  (атаки огибающей), а сам эффект делать детерминированным.
...
- Пиксель-арт пробовался и ОТКАТАН целиком по решению пользователя: спрайты
  голов и перчаток, десять вариантов внешности и пиксельный конвейер кадра
  (крупный тексель, постеризация). Персонажи остаются «круглыми», векторными.
  Код всего этого есть в истории ветки; возвращать только по просьбе.
