...
(async()=>{
  ...
  const r=await p.evaluate(async ()=>{
    ...
    let mx=0, dmax=0, moves=0, liftedBefore=0, gap=999, starts=0, baseMoved=0; const lifts=[];
      ...
      for(const kk of [0,1,2,3]){ const k=kk%2, fld=(kk<2?'d':'e')+k;
        ...
        if(dd>0.01){ moves++;
          if(Math.abs(pr[fld]-tr[Math.max(0,i-2)][fld])<=0.01){ starts++;
            ...
            let lift=0; for(let j=i;j<Math.min(tr.length,i+45);j++) lift=Math.max(lift,tr[j]['y'+k]);
            // порог отделяет отрыв от волочения: при 60 кадрах подъём ~1.5,
            // под нагрузкой выборка реже и пик срезается до ~0.6
            lifts.push(+lift.toFixed(2)); if(lift>0.4) liftedBefore++; } } } }
    ...
    return {кадров:tr.length, смещение_ступни_макс:+mx.toFixed(2), скорость_ступни_макс:+dmax.toFixed(0), метка_сдвинулась:+baseMoved.toFixed(2), кадров_движения:moves, переступов:starts, подъёмы:lifts,
            стопа_отрывалась:liftedBefore, зазор_стоп_мин:+gap.toFixed(1), база_в_размене:new Set(bases).size};
  });
  console.log(JSON.stringify(r));
  const ok = r.смещение_ступни_макс<=3.01 && r.метка_сдвинулась===0 && r.переступов>=1 && r.стопа_отрывалась===r.переступов && r.зазор_стоп_мин>=6 && r.база_в_размене===1;
