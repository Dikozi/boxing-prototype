...
(async()=>{
  ...
  const r=await p.evaluate(async ()=>{
    ...
    for(let j=0;j<2;j++){ let prev={head:0,body:0};
      for(let n=0;n<=N;n++){ const m=marksUpTo(j,n); series[j].push(m.head+'/'+m.body+(m.cut?'*':''));
        if(m.head<prev.head||m.body<prev.body) mono[j]=false; prev=m; } }
    // живой раунд: следы в конце размена совпадают с формулой по записям
    let liveOk=true, checked=0;
    ...
    for(let k=0;k<N && checked<3;k++){
      ...
      const end=[0,1].map(j=>JSON.stringify(B[j].marks)), want1=[0,1].map(j=>JSON.stringify(marksUpTo(j,k+1)));
      if(start.join()!==want0.join() || end.join()!==want1.join()) liveOk=false;
      checked++;
    }
    marksIdle(); driveIdle(); for(let n=0;n<30;n++) settle(1/60);
    render(); paintRing();
    const fin=[marksUpTo(0,N), marksUpTo(1,N)];
    return {раундов:N, монотонно:mono, ряд0:series[0].join(' '), ряд1:series[1].join(' '), живой_совпал:liveOk, проверено_раундов:checked,
  ...
  const ok = r.монотонно[0] && r.монотонно[1] && r.живой_совпал && r.проверено_раундов>=2 && r.есть_следы;
