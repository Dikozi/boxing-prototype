...
(async()=>{
  ...
  const r=await p.evaluate(async ()=>{
    ...
    for(const [id,dmg,lvl] of CASES){
      ...
      const D=B[1]; let h0=null, ln0=null, away=0, toward=0, lean=0, rag=false, sprayMax=0;
      playRound(rc,()=>{});
      await new Promise(res=>{ const tick=()=>{ if(!seq) return res();
        ...
        if(D.rd) rag=true;
        sprayMax=Math.max(sprayMax, parts.filter(q=>q.s).length);   // брызги слюны
      ...
      out.push({удар:id, уровень:lvl, урон:dmg, голова_от:+away.toFixed(1),
                голова_навстречу:+toward.toFixed(1), наклон:+lean.toFixed(2), рагдолл:rag, брызг:sprayMax});
    }
    return out;
  });
  console.log('удар'.padEnd(20)+'урон  голова ОТ  навстречу  наклон  рагдолл  брызг');
  for(const x of r) console.log((x.удар+'/'+x.уровень).padEnd(20)+String(x.урон).padStart(4)+
    ...
    String(x.наклон).padStart(8)+(x.рагдолл?'   да':'   нет')+String(x.брызг).padStart(7));
  const sprayOk = r.every(x => (x.уровень==='head' && x.урон>=24) ? x.брызг>=6 : x.брызг===0);
  console.log(sprayOk?'✔ брызги: только на сильном ударе в голову':'✖ брызги: не там или не столько');
