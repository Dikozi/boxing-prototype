...
(async()=>{
  ...
  const r=await p.evaluate(async ()=>{
    ...
    for(const id of ['jab','hook','crossLand','kick','spinLand']){
      ...
      const A=B[0]; let boneErr=0, handZAtHit=null, layerOk=null, footZmin=99, footZmax=-99, shDy=null, handZmax=-99, hookLayer=null, shZhit=null;
      ...
      await new Promise(res=>{ const tick=()=>{ if(!seq) return res();
        ...
        if(handZAtHit===null && seq.t>=seq.imp){
          handZAtHit=J.arms[lead].hand.z||0; shZhit=[+(J.arms[0].sh.z||0).toFixed(1), +(J.arms[1].sh.z||0).toFixed(1)];
          ...
          // кросс: бьющее (дальнее) плечо выходит на линию удара, переднее уходит к зрителю — это и есть поворот
          layerOk = (id==='crossLand') ? (Math.abs(shZhit[1])<=1 && shZhit[0]>=4) : (id==='kick' ? (leg && torso && leg.z>torso.z) : true);
        }
      ...
      out[id]={кости_макс_ошибка:+boneErr.toFixed(3), z_кулака_в_контакте:+handZAtHit.toFixed(1), слой_выше_корпуса:layerOk,
               ...
               z_кулака_макс:+handZmax.toFixed(1), рука_над_корпусом_в_размахе:hookLayer, z_плеч_в_контакте:shZhit};
    }
    return out;
  });
  ...
  console.log(ok?'✔ глубина: кости целы в пространстве, кросс поворотом к линии удара, хук вокруг корпуса, нога перед корпусом, вертушка за ним':'✖ глубина: вне порогов', ' ошибок:', errs.length, errs.slice(0,2));
  await b.close();
})();
