...
(async()=>{
  ...
  const r=await p.evaluate(async ()=>{
    ...
    const out={};
    for(const id of ['jab','crossLand','kick','spinLand']){
      ...
      const A=B[0]; let boneErr=0, handZAtHit=null, layerOk=null, footZmin=99, footZmax=-99, shDy=null;
      playRound(rc,()=>{});
      await new Promise(res=>{ const tick=()=>{ if(!seq) return res();
        ...
        if(handZAtHit===null && seq.t>=seq.imp){
          handZAtHit=J.arms[id==='jab'?0:1].hand.z||0;
          const list=[]; boxerParts(A, list);
          const torso=list.find(e=>e.name==='torso'), arm=list.find(e=>e.name===(id==='jab'?'nearArm':'farArm')), leg=list.find(e=>e.name==='farLeg');
  ...
  console.log(ok?'✔ глубина: кости целы в пространстве, кросс к линии удара, нога перед корпусом, вертушка за ним':'✖ глубина: вне порогов', ' ошибок:', errs.length, errs.slice(0,2));
  await b.close();
})();
